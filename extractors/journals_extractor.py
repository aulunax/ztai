...
class JournalsExtractor(Extractor):
    ...
    def _compute_newline_separators(
        ...
        for line in page.lines:
            ...
            if abs(float(linewidth) - JOURNALS_NEWLINE_LINEWIDTH) > 1e-6:
                continue
    ...
    def _compute_cutoffs(self, page: pdfplumber.page.Page) -> tuple[float | None, float | None]:
        ...
        for line in page.lines:
            linewidth = line.get("linewidth")
            if linewidth is None or abs(float(linewidth) - JOURNALS_SEPARATOR_LINEWIDTH) > 1e-6:
                continue
            line_top = self._line_pos_from_top(line, page_height)
            if line_top is None:
                continue
    ...
    def _filter_chars(self, page: pdfplumber.page.Page) -> list[dict]:
        ...
        for char in page.chars:
            ...
            if (
                ...
                and size < JOURNALS_ANNOTATION_MAX_SIZE
                ...
                and (
                    ...
                    (float(base_y0) - float(prev_base_y0)) >= JOURNALS_ANNOTATION_BASELINE_DELTA
                    or 
                    # Subscript check: current top (y1) is significantly lower than previous
                    (float(prev_base_y1) - float(base_y1)) >= JOURNALS_ANNOTATION_BASELINE_DELTA
                )
            ):
                count_annotation += 1
                continue
    ...
    def _filter_table_blocks(
        ...
        for line in page_lines:
            if JOURNALS_TABLE_START_RE.match(line["text"]):
                table_start = line["top"]
                is_in_table_block = True
            if JOURNALS_SOURCE_RE.match(line["text"]):
                table_end = line["top"]
    ...
    def _merge_by_size(
        ...
        for char_index, char in enumerate(chars):
            ...
            size_changed = (
                ...
                or abs(size - current_size) > JOURNALS_SIZE_TOLERANCE
            )
            ...
            if size_changed or bold_changed:
                ...
                if current_text:
                    line_text = "".join(current_text).strip()
                    if line_text:
                        if JOURNALS_CITATION_STOP_RE.search(line_text):
                            merged_lines.append((JOURNALS_CITATION_STOP_MARKER, last_page_index or page_index))
                        else:
                            merged_lines.append((line_text, last_page_index or page_index))
        ...
        if current_text:
            line_text = "".join(current_text).strip()
            if line_text and JOURNALS_CITATION_STOP_RE.search(line_text):
                merged_lines.append((JOURNALS_CITATION_STOP_MARKER, last_page_index or page_index))
                current_text = []
                current_chars = []
    ...
    def _truncate_on_markers(self, lines: list[tuple[str, int]]) -> tuple[list[tuple[str, int]], bool]:
        ...
        for line, page_no in lines:
            if line == JOURNALS_CITATION_STOP_MARKER:
                return kept, True
            normalized = " ".join(line.lower().split())
            normalized_slash = normalized.replace(" / ", "/")
            if any(marker in normalized for marker in JOURNALS_STOP_MARKERS) or any(
                marker.replace(" / ", "/") in normalized_slash for marker in JOURNALS_STOP_MARKERS
            ):
                return kept, True
    ...
    def _extract_text_from_pdf(self, pdf_path: Path) -> str:
        with pdfplumber.open(pdf_path) as pdf:
            ...
            for page_index, page in enumerate(pdf.pages, start=1):
                ...
                if page_index == 1:
                    with Path("test_aa.json").open("w", encoding="utf-8") as f:
                        json.dump(page.rects, f, ensure_ascii=False, indent=2)
            ...
            log_path = self.output_dir / JOURNALS_LANGDETECT_LOG_NAME
